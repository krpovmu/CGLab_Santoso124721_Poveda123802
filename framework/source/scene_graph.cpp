...
SceneGraph::SceneGraph(std::string const &name)
    ...
    , rootNode_(std::make_shared<Node>(Node()))
{}
SceneGraph::SceneGraph(std::string const &name, std::shared_ptr<Node> const &rootNode)
    : name_(name)
    , rootNode_(rootNode)
{}
...
std::string SceneGraph::printNode(std::shared_ptr<Node> const &node) const
{
    ...
    std::string outputString
        = node->getName() /* + "(" + glm::to_string(node->getWorldTransform()) + ")"*/;
